...
public class Testpool {
    ...
    public void testDruidPoolConfiguration() {
        ...
        System.out.println("连接数(InitialSize): " + 3);
        ...
        try {
            ...
            System.out.println("获取第3个连接成功");

            // 获取第4个连接（达到最大活跃连接数）
            Connection conn4 = druid.getConnection();
            connections.add(conn4);
            System.out.println("获取第4个连接成功（达到最大活跃连接数）");

            // 尝试获取第5个连接（应该会等待或超时）
            try {
                System.out.println("尝试获取第5个连接（超过最大活跃连接数）...");
                Connection conn5 = druid.getConnection();
                connections.add(conn5);
                System.out.println("获取第5个连接成功");
            } catch (SQLException e) {
                System.out.println("获取第5个连接失败，原因: " + e.getMessage());
            }
    ...
    public void testConcurrentConnections() {
        ...
        // 创建多个线程同时获取连接
        Thread[] threads = new Thread[8]; // 6个线程，超过最大连接数4
